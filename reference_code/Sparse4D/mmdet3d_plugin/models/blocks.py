...
@ATTENTION.register_module()
class BEVFeatureAggregation(BaseModule):
    """从 BEV 特征图上根据 anchor 的 (x,y) 采样特征，用于 Sparse4D head 接 BEV 的场景。

    与 DeformableFeatureAggregation 接口一致，但 feature_maps 为单张 BEV 图
    (B, C, H_bev, W_bev)，通过 key_points 的 x,y 投影到 BEV 网格并 grid_sample。
    """
    ...
    def __init__(
        ...
        super(BEVFeatureAggregation, self).__init__()
        self.embed_dims = embed_dims
        self.proj_drop = nn.Dropout(proj_drop)
        self.residual_mode = residual_mode
        ...
        self.output_proj = Linear(self.embed_dims, self.embed_dims)

    def init_weight(self):
        xavier_init(self.output_proj, distribution="uniform", bias=0.0)

    def _key_points_to_bev_grid(self, key_points: torch.Tensor) -> torch.Tensor:
        """key_points (bs, num_anchor, num_pts, 3) -> BEV 归一化坐标 (bs, num_anchor, num_pts, 2) in [-1,1]."""
        bs, num_anchor, num_pts, _ = key_points.shape
        xy = key_points[..., :2]
        xmin, xmax, dx = self.bev_bounds[0, 0].item(), self.bev_bounds[0, 1].item(), self.bev_bounds[0, 2].item()
        ymin, ymax, dy = self.bev_bounds[1, 0].item(), self.bev_bounds[1, 1].item(), self.bev_bounds[1, 2].item()
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        if self.kps_generator is not None:
            ...
            key_points = anchor[..., [0, 1, 2]].unsqueeze(2)
