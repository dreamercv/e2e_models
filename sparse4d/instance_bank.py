"""
InstanceBank 纯 PyTorch 实现，不依赖 mmcv。
"""
import numpy as np
import torch
...
def topk(confidence, k, *inputs):
    ...
    outputs = []
    for t in inputs:
        outputs.append(t.flatten(end_dim=1)[indices].reshape(bs, k, -1))
...
class InstanceBank(nn.Module):
    def __init__(
        ...
        num_anchor: int,
        embed_dims: int,
        anchor,  # np.ndarray or path or list
        anchor_handler=None,
        ...
        self.anchor = nn.Parameter(torch.tensor(anchor, dtype=torch.float32), requires_grad=anchor_grad)
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        instance_feature = self.instance_feature[None].expand(batch_size, -1, -1).clone()
        anchor = self.anchor[None].expand(batch_size, -1, -1).clone()

        # 无时序：不做 cache / dn_anchor 投影，直接返回
        if self.num_temp_instances <= 0:
            time_interval = instance_feature.new_tensor([self.default_time_interval] * batch_size)
            return (instance_feature, anchor, None, None, time_interval)
    ...
    def get_instance_id(self, confidence, anchor=None, threshold=None):
        ...
        new_ids = torch.arange(num_new_instance, device=instance_id.device) + self.prev_id
    ...
    def update_instance_id(self, instance_id=None, confidence=None):
        if self.temp_confidence is None:
            temp_conf = confidence.max(dim=-1).values if confidence.dim() == 3 else confidence
        else:
