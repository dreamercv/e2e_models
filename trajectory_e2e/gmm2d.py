...
class GMM2D(td.Distribution):
    ...
    def __init__(self, log_pis, mus, log_sigmas, corrs):
        ...
        log_pis, mus, log_sigmas, corrs = log_pis.float(), mus.float(), log_sigmas.float(), corrs.float()
        self.components = log_pis.shape[-1]
        self.dimensions = 2
        self.device = log_pis.device
        log_pis = torch.clamp(log_pis, min=-1e5)
        # log_pis = torch.clamp(log_pis, min=-1e4)
...
class Dynamic(object):
    def __init__(self, dt, dyn_limits, device, node_type):
        self.dt = dt
        self.device = device
        self.dyn_limits = dyn_limits
        self.initial_conditions = None
